...
from lwgnet.utils import alpha_blending
...
class test_reconstruction(Base_configuration):
    def __init__(
        ...
        super().__init__(device, root_dir, data_config, model_config, model_params, model)
    ...
    def _test_epoch(self):
        ...
        test_filenames = [
            line.rstrip("\n")
            for line in pathlib.Path(os.path.join(self.root_dir, self.data_config["test_filenames"])).open()
        ]
